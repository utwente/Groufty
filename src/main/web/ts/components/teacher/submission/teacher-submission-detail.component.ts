...
import {StudentReviewEditor} from "../../student/review-editor/student-review-editor";

@Component({
    templateUrl: './templates/teacher/submission/teacher-submission-detail.component.html',
    directives: [TeacherSubmissionDetailComponent, StudentReviewEditor, PDFComponent, MarkdownComponent, AlertComponent],
providers: [TaskDetailService],
    pipes: [MomentPipe]
})
export class TeacherSubmissionDetailComponent implements OnInit{
    // Error bool
    ...
    public ngOnInit() : void {
        var taskId = parseInt(this._routeParams.get('tid'));
	    var authorId = parseInt(this._routeParams.get('aid'));
	    
        this.getTaskDetailsById(taskId, authorId);
    }

    public getTaskDetailsById(taskId : number, authorId : number) : void {
        this._taskDetailService.getTeacherTaskDetails(taskId, authorId).subscribe(
            taskDetail => {
